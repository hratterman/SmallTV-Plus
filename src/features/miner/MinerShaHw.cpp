...
static inline void hwFillFirstBlock(const void* text) {
...
static inline void hwFillSecondBlock(const void* text, uint32_t nonce) {
  const uint32_t* w = (const uint32_t*)text;
  uint32_t* reg = (uint32_t*)(SHA_TEXT_BASE);
  reg[0]  = w[0];
  reg[1]  = w[1];
...
static inline void hwFillDoubleBlock() {
  uint32_t* reg = (uint32_t*)(SHA_TEXT_BASE);
  reg[8]  = 0x80000000;
...
static inline bool hwReadDigestSwapped(void* out, bool earlyExit) {
...
static inline bool hwDoubleHash(const uint8_t* swapped128, uint32_t nonce,
