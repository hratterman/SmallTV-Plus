...
def generate():
    ...
    # Zopfli when available: an exhaustive, gzip-compatible encoder that finds
    # ~4% more than gzip -9. On a 23 KB page that is ~900 bytes - which once
    # was exactly the margin by which the app image outgrew the OTA ceiling a
    # deployed build would accept. CI installs it; a bare checkout falls back.
    try:
        from zopfli.gzip import compress as _zopfli
        packed = _zopfli(raw, numiterations=600)
    except ImportError:
